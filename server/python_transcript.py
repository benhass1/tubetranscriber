...
def _language_info(code: str, name: Any = None, *, original: bool = False, asr: bool = False) -> dict[str, Any]:
    ...
    return {
        "code": normalized,
        "name": _language_name(name, normalized or "Unknown language"),
        "isOriginal": original,
        "isAutoGenerated": asr,
    }
...
def _translation_languages(player_data: dict[str, Any]) -> list[dict[str, Any]]:
    captions = player_data.get("captions", {})
    renderer = captions.get("playerCaptionsTracklistRenderer", {}) if isinstance(captions, dict) else {}
    values = renderer.get("translationLanguages", []) if isinstance(renderer, dict) else []
...
def _result(
    ...
    selected_code = str(selected_track.get("languageCode") or selected_track.get("lang_code") or original_code).strip()
    available = _languages_from_tracks(tracks, translations, original_code)
    selected_language = next((item for item in available if item["code"] == selected_code), _language_info(
        selected_code,
        selected_track.get("name"),
        original=selected_code == original_code,
        asr=str(selected_track.get("kind", "")).lower() == "asr",
    ))
    original_language = next((item for item in available if item["code"] == original_code), _language_info(original_code, original_track.get("name"), original=True))
    return {
        "segments": segments,
...
def _with_translation(track: dict[str, Any], language_code: str | None) -> dict[str, Any]:
    target = str(language_code or "").strip()
    if not target or target.lower() == str(track.get("languageCode", "")).strip().lower():
        return track
    ...
    if base_url:
        ...
        translated["baseUrl"] = urlunsplit((parts.scheme, parts.netloc, parts.path, urlencode(query), parts.fragment))
    translated["languageCode"] = target
    translated["name"] = target
    translated["translatedFrom"] = str(track.get("languageCode", ""))
    return translated
    ...
    try:
        ...
        for config in _innertube_client_configs(token_details):
            try:
                ...
                if tracks:
                    return tracks, _translation_languages(player_data)
            except YouTubeRateLimitError:
                raise
...
def _select_original_caption_track(tracks: list[dict[str, Any]]) -> dict[str, Any]:
    original = next((track for track in tracks if str(track.get("baseUrl", "")).strip()), None)
    if not original:
        raise NoCaptionsError("No public captions are available for this video.")
    return original


def _select_language_track(tracks: list[dict[str, Any]], language_code: str | None) -> tuple[dict[str, Any], dict[str, Any]]:
    original = _select_original_caption_track(tracks)
    target = str(language_code or "").strip().lower()
    if not target or target == str(original.get("languageCode", "")).strip().lower():
        return original, original
    exact = next((track for track in tracks if str(track.get("languageCode", "")).strip().lower() == target), None)
    return original, exact or _with_translation(original, language_code)
...
def get_transcript_payload(url_or_id: str, language_code: str | None = None) -> dict[str, Any]:
    """Extract a transcript and return text, segments, and language metadata."""
    video_id = extract_video_id(url_or_id)
    result = _fetch_transcript_result(video_id, language_code)
...
def main() -> None:
    ...
    try:
        ...
        print(json.dumps(get_transcript_payload(url_or_id, str(language_code).strip() if language_code else None)))
