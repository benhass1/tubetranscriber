...
class TranscriptBridgeTests(unittest.TestCase):
    ...
    def test_result_chain_reaches_dynamic_key_fallback_after_catalog_and_library_fail(self):
        expected = {
        ...
        with patch.object(bridge, "_fetch_transcript_in_language", side_effect=RuntimeError("catalog failed")), patch.object(
            bridge, "_youtube_transcript_api_fetch_result", side_effect=RuntimeError("library failed")
        ), patch.object(bridge, "_fetch_innertube_api_key_result", return_value=expected) as fallback:
