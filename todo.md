...
- [x] Assess whether the successful sandbox-local network can serve live public transcript requests: the default sandbox hibernates and cannot provide a stable public backend or durable egress IP. A visitor-local Python app, or a user-owned always-on machine reached through a secure tunnel, is the only no-managed-provider option that can make requests from a non-cloud network; neither guarantees YouTube will permit every request.
- [x] Build a self-contained visitor-local Python TubeTranscriber companion that runs only on localhost, retrieves captions through the user’s connection, supports plain-text reading and TXT/JSON/SRT exports, and documents installation and launch steps. Validation passed: 5 local Python tests, 15 web tests, TypeScript check, loopback-only smoke test, and successful real retrieval of kTvNFp3N9T0 from the local companion endpoint.
- [x] Assess whether an embedded YouTube player can expose YouTube’s viewer-visible “Show transcript” content to TubeTranscriber: YouTube’s IFrame API supports playback control but does not provide a transcript-panel read method, and browser same-origin isolation prevents a third-party page from reading or clicking the YouTube frame’s DOM. The supported local-app alternative is to render an embedded player beside transcript segments independently retrieved by the visitor-local Python companion, then use the player API only for seek/playback synchronization.
- [x] Review jaypaun007/youtube-transcript-api for license, source behavior, dependencies, and cloud-host compatibility. Do not integrate it: its public tree contains only README and Apache-2.0 license files, its live API contract differs from the documented request body, and its live response for kTvNFp3N9T0 was not a usable transcript. Retain the tested local Python companion instead.
- [x] Review the official jdepoix/youtube-transcript-api project for current behavior, maintenance, license, and cloud-host implications. TubeTranscriber already uses its current v1.2.4 release and supported `list()`/`fetch()` interfaces; no dependency or adapter replacement is needed, and the library’s documented cloud-IP blocks match the published-site limitation.
- [x] Convert TubeTranscriber to a yt-dlp-only extraction architecture across the hosted server, visitor-local companion, dependencies, tests, and documentation; remove the youtube-transcript-api runtime path without adding a managed provider, proxy, cookie import, or access-control bypass. Validation passed: 7 local companion tests, 13 web tests, TypeScript, and production build.
- [x] Prepare the yt-dlp-only visitor-local companion for subsequent secure Windows-PC worker integration while preserving localhost-only behavior until the worker tunnel is explicitly configured. It now declares yt-dlp curl-cffi support and Node 22, but the sandbox’s current IP remains YouTube-restricted for kTvNFp3N9T0, so the Windows-network test remains necessary.
- [ ] Connect the user’s always-on Windows PC as an authenticated yt-dlp-only worker behind a secure tunnel after the user makes that PC available for setup.
