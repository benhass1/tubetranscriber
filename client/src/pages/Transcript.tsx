...
export default function Transcript() {
    ...
    lookup.mutate({ url: sourceUrl }, {
      onError: async error => {
        ...
        try {
          ...
          if (cancelled) return;
          setBrowserFallbackError(fallbackError instanceof Error ? fallbackError.message : "Browser extraction was not available for this video.");
          if (retryNonce < 2) return;
          ...
          try {
            ...
            if (!cancelled) setContaboFallbackError(contaboError instanceof Error ? contaboError.message : "The Contabo fallback was not available.");
          } finally {
  ...
  const effectiveError = contaboFallbackError || browserFallbackError || (browserData ? "" : lookup.error?.message || "");
  ...
  if (lookup.isPending || browserFallbackPending || contaboFallbackPending || (!data && !effectiveError)) return <SiteShell><TranscriptLoading browserFallbackPending={browserFallbackPending || contaboFallbackPending} /></SiteShell>;
