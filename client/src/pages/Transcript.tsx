...
import { AlertCircle, ArrowRight, Check, ChevronLeft, Clipboard, Download, ExternalLink, FileJson, FileText, Link2, Loader2, RotateCcw, Search, Subtitles, X } from "lucide-react";
...
export default function Transcript() {
  ...
  const contaboFallback = trpc.transcript.localFallback.useMutation();
  const pcFallback = trpc.transcript.pcFallback.useMutation();
  ...
  const [contaboFallbackPending, setContaboFallbackPending] = useState(false);
  const [contaboFallbackError, setContaboFallbackError] = useState("");
  const [pcFallbackPending, setPcFallbackPending] = useState(false);
  const [pcFallbackError, setPcFallbackError] = useState("");
  ...
  useEffect(() => {
    if (!sourceUrl) return;
    if (previousSourceUrl.current !== sourceUrl) previousSourceUrl.current = sourceUrl;
    let cancelled = false;
    lookup.reset();
    browserIngest.reset();
    contaboFallback.reset();
    pcFallback.reset();
    setBrowserData(undefined);
    setBrowserFallbackError("");
    setBrowserFallbackPending(false);
    setContaboFallbackPending(false);
    setContaboFallbackError("");
    setPcFallbackPending(false);
    setPcFallbackError("");

    const acceptResult = (result: BrowserTranscriptResult) => {
      if (cancelled) return;
    ...
    const runRetryAttempt = async () => {
      setPcFallbackPending(true);
      try {
        const result = await pcFallback.mutateAsync({ url: sourceUrl }) as BrowserTranscriptResult;
        acceptResult(result);
      } catch (error) {
        if (!cancelled) setPcFallbackError(error instanceof Error ? error.message : "The PC fallback was not available.");
        try {
          const result = await lookup.mutateAsync({ url: sourceUrl }) as BrowserTranscriptResult;
          acceptResult(result);
        } catch (renderError) {
          if (!cancelled) setPcFallbackError(renderError instanceof Error ? renderError.message : "Render could not retrieve this transcript.");
        }
      } finally {
        if (!cancelled) setPcFallbackPending(false);
      }
    };

    void (retryNonce === 0 ? runFirstAttempt() : runRetryAttempt());
