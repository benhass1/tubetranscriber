...
function friendlyTranscriptError(error: string) {
  ...
  if (normalized.includes("cloudflare stage") || normalized.includes("transcript unavailable") || normalized.includes("bad gateway")) {
    return "Sometimes it works on the second try. YouTube's blocking is random.";
  }
...
export default function Transcript() {
  ...
  useEffect(() => {
    if (!sourceUrl) return;
    const sourceChanged = Boolean(previousSourceUrl.current) && previousSourceUrl.current !== sourceUrl;
    if (sourceChanged) {
      previousSourceUrl.current = sourceUrl;
      scrolledVideoIdRef.current = "";
    } else if (!previousSourceUrl.current) {
      previousSourceUrl.current = sourceUrl;
    }
    let cancelled = false;
    lookup.reset();
