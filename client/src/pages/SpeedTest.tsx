...
export default function SpeedTest() {
  ...
  async function handleStartTest() {
    ...
    const gaugeTarget = gaugeRef.current;
    if (gaugeTarget) {
      const isPhoneOrTablet = window.matchMedia?.("(max-width: 1024px)").matches ?? false;
      window.requestAnimationFrame(() => gaugeTarget.scrollIntoView({ behavior: "smooth", block: isPhoneOrTablet ? "start" : "center" }));
    }
    setTestState("ping");
  ...
  return (
    <SiteShell>
      <div className="speed-test-page">
        <section className="speed-test-live-section">
          <div className="content-container speed-test-live-container">
            ...
            <div className="speed-gauge-layout">
              ...
              <div ref={gaugeRef} id="speed-test-results-container" className="speed-gauge-wrap" aria-label={`${gaugeLabel} live speed gauge`}>
        ...
        <section className="speed-workflow-section content-container" aria-labelledby="workflow-heading"><p className="eyebrow">Next step for creators</p><h2 id="workflow-heading">Optimizing Your Creator Workflow</h2><p>Your upload test helps plan publishing, while the <a href="/">free YouTube transcript generator</a> turns public captions into a searchable working document. When you need to review a tutorial, interview, or Short, use TubeTranscriber to <a href="/">convert YouTube video to searchable text</a> and export the result in TXT, SRT, or JSON.</p></section>
      </div>
    </SiteShell>
  );
}
